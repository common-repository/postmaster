...
for ($i=1; $i <= $count; $i++) :
	...
	foreach ($message as $line) :
		...
		if ($bodysignal) {
			...
			if ( preg_match('/(From|Reply-To): /', $line) )  {
				...
				if ( is_email($author) ) {
					// Modified by Brett Duncavage For PostMaster
	...
	// For Debugging
	//echo "Applying Filter<br />";
	//echo "Subject: $subject<br />";
	//

	// Modified by Brett Duncavage for PostMaster
	// need to implode the message back to its original state in order
	// for PostMaster to correctly process it.
	$post_content = apply_filters('phone_content', implode($message));
